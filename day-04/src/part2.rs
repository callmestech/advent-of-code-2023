...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    let (_, cards) = parse_games(input).expect("Should parse input");
    ...
    let result = matches
        .iter()
        .enumerate()
        .fold(store, |mut acc, (index, num_matches)| {
            let to_add = *acc.get(&index).unwrap();

            for i in (index + 1)..(index + 1 + *num_matches) {
...
fn card(input: &str) -> IResult<&str, Card> {
    ...
    separated_pair(set, tuple((tag("|"), space1)), set)
        .map(|(winning_numbers, numbers)| Card {
            winning_numbers,
            numbers,
        })
        .parse(input)
...
fn parse_games(input: &str) -> IResult<&str, Vec<Card>> {
